<script lang="ts">
	...
	const statusKey = $derived(
		$t(`projects.items.${project.slug}.status`) === 'live'
			? 'statusLive'
			: $t(`projects.items.${project.slug}.status`) === 'prototype'
				? 'statusPrototype'
				: 'statusDevelopment'
	);
...
<SEO
	...
	breadcrumbs={[
		...
		{ name: 'Portfolio', url: `https://bigls.net/${data.lang}/#projects` },
		{ name: deProject.title ?? project.slug, url: `https://bigls.net/${data.lang}/projects/${project.slug}` }
	]}
/>
...
>
	...
	>
		...
		<div class="relative z-10 max-w-6xl mx-auto w-full">
			...
			>
				{$t(`projects.items.${project.slug}.subtitle`)}
			</h2>
			...
			>
				<div class="hero-meta-item min-w-0">
					<span
						...
						>{$t('projectDetail.metaRole')}</span
					>
					<span class="text-xs sm:text-sm font-medium" style="color: var(--text-secondary);"
						>{$t(`projects.items.${project.slug}.role`)}</span
					>
				</div>
				<div class="hero-meta-item">
					<span
						class="block text-[9px] sm:text-[10px] font-bold uppercase tracking-[0.2em] sm:tracking-[0.25em] text-blue-500/40 mb-1 sm:mb-1.5 font-syne"
						>{$t('projectDetail.metaTimeline')}</span
					>
					<span class="text-xs sm:text-sm font-medium" style="color: var(--text-secondary);">
						<time>{$t(`projects.items.${project.slug}.timeline`)}</time>
					</span>
				</div>
				<div class="hero-meta-item">
					<span
						class="block text-[9px] sm:text-[10px] font-bold uppercase tracking-[0.2em] sm:tracking-[0.25em] text-blue-500/40 mb-1 sm:mb-1.5 font-syne"
						>{$t('projectDetail.metaType')}</span
					>
					<span class="text-xs sm:text-sm font-medium" style="color: var(--text-secondary);"
						>{$t(`projects.items.${project.slug}.type`)}</span
					>
				</div>
			</div>
		</div>
		...
		>
			...
			>
				...
				{#if typeof project.image === 'object'}
					<enhanced:img
						src={project.image}
						alt={$t('a11y.projectImageAlt', { project: $t(`projects.items.${project.slug}.title`) } as any)}
	...
	{#if $t(`projects.items.${project.slug}.sections`)}
			...
			>
				<div class="grid lg:grid-cols-12 gap-6 sm:gap-8 lg:gap-16 items-start">
					...
					<div class="lg:col-span-8">
						...
						>
							{$t(`projects.items.${project.slug}.sections.context`)}
						</p>
					</div>
				</div>
			</article>
			...
			>
				<div class="grid lg:grid-cols-12 gap-6 sm:gap-8 lg:gap-16 items-start">
					...
					>
						...
						>
							{$t('projectDetail.chapterChallenge')}
						</h3>
					</header>
					<div class="lg:col-span-8 lg:order-1">
						...
						>
							{$t(`projects.items.${project.slug}.sections.problem`)}
						</p>
					</div>
				</div>
			</article>
				...
				<div class="grid lg:grid-cols-12 gap-6 sm:gap-8 lg:gap-16 items-start">
					...
					<div class="lg:col-span-8">
						<p
							...
							style="color: var(--text-secondary);"
						>
							{$t(`projects.items.${project.slug}.sections.solution`)}
						</p>
					</div>
				</div>
			</article>
				...
				>
					...
					<div class="relative z-10">
						<header class="flex items-end gap-4 sm:gap-6 mb-6 sm:mb-8">
							...
							>
								{$t('projectDetail.chapterOutcome')}
							</h3>
						</header>
						...
						<p
							...
							style="color: var(--text-heading); opacity: 0.9;"
						>
							{$t(`projects.items.${project.slug}.sections.impact`)}
						</p>
					</div>
				</section>
			</article>
	...
	>
		<div class="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-8 lg:gap-16">
			...
			<div class="tech-stack-row flex flex-wrap gap-2 sm:gap-3">
				{#each ($t(`projects.items.${project.slug}.technologies`) as any) ?? [] as tech}
					<span
	...
	{#if $t(`projects.items.${project.slug}.features`)}
		<section class="relative z-10 max-w-6xl mx-auto px-5 sm:px-8 lg:px-16 pb-20 sm:pb-32 lg:pb-40">
			...
			>
				{#each ($t(`projects.items.${project.slug}.features`) as any) ?? [] as feature, i}
					<article
