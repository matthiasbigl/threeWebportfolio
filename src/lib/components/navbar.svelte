<script lang="ts">
	...
	async function toggleMenu() {
		...
		if (browser && isHamOpen && !$reducedMotion) {
...
<header
	bind:this={navbar}
	class="navbar fixed top-0 left-0 z-50 w-full flex items-center justify-center px-4 sm:px-6 py-3 sm:py-4 md:py-2 transition-all duration-300"
	...
	<div class="w-full max-w-6xl flex items-center justify-between">
		...
		{#if isSubPage}
			...
			>
				...
				<span class="text-[10px] sm:text-[11px] font-medium tracking-[0.15em] uppercase font-syne"
					>{backLabel}</span
				>
			</a>
		{:else}
...
{#if !isSubPage && isHamOpen}
	...
	<div
		class="mobile-menu fixed top-0 left-0 w-full h-full backdrop-blur-md flex items-center justify-center z-50"
